...
pubDate: 'Aug 24 2025'
tags: ['big-button', 'embedded', 'rust', 'cargo', 'github', 'configs']
draft: false
tech: true
series: {
...
<Tag color="green" txt="💻 HACKER" /> Once we start adding all kinds of code, logic, and structure to our
...
```toml title="Cargo.toml" collapse={1-46, 55-67}
########################################################################################
...
```rust title="build.rs" ins={6, 8-9} {12}
...
```rust title="build.rs" {1, 11} ins={13-23}"
// ^^^ previous existing code ^^^
...
```rust title="build.rs" collapse={1-12} ins={27-31} {34}
//! This build script runs during code compilation
//! Runs on host computer, where `std` Rust tools are available here
//! Reference: https://doc.rust-lang.org/cargo/reference/build-scripts.html
...
```rust title="src/main.rs" {26} collapse={1-13} ins={15-18, 26-27}
#![no_std]
#![no_main]
...
```rust title="src/main.rs" {1, 6} ins={11-21}
...
.main-none-eabihf/release/big-button`
      Erasing ✔ 100% [####################]  12.00 KiB @  54.08 KiB/s (took 0s)
     Finished in 0.61s
