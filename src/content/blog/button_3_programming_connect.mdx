...
<Aside variant="note">
  On the debug probe, we are only connecting the DAP (Debug Adapter Protocol) (*marked as `D`*)
  connection and not the Serial UART connection (*marked as `U`*). We will be not be
  using the serial wire to upload anything to the Pico.
</Aside>
