...
## Introduction
<Tag color="green" txt="💻 HACKER" /> We spend some time setting up our code structure to where all the things start up
...
src
├── button
│   ├── consumer_loop.rs  # <--- Changes
│   ├── core.rs
│   ├── messaging.rs      # <--- Changes
│   ├── mod.rs            # <--- Changes
│   └── utility.rs        # <--- New
├── clocks_config.rs
├── main.rs               # <--- Changes
├── state_machine.rs      # <--- Changes
├── system_manager.rs     # <--- New
└── utility.rs
```
