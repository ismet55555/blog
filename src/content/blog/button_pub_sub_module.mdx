...
description: 'Implement Publisher-Subscriber messaging pattern, adding button code, and organize code with Rust modules.'
pubDate: 'Nov 15 2025'
...
## Introduction
<Tag color="green" txt="💻 HACKER" /> In this part,
we’ll focus on setting up a fitting **code design pattern** for our button. 
...
**[State pattern](https://refactoring.guru/design-patterns/state)**, where the program’s flow 
...
even worse, one single file. Wouldn't that be fun?! 😭
...
<Tag color="blue" txt="💻 MAKER" /> By the end of this part we will have code that works with a button press input.
...
```rust title="src/button/messaging.rs" showLineNumbers startLineNumber=1 collapse={} ins={} 
...
```rust title="src/button/messaging.rs" showLineNumbers startLineNumber=16 collapse={} ins={} 
...
```rust title="src/button/messaging.rs" showLineNumbers startLineNumber=29 collapse={} ins={} 
...
```rust title="src/button/core.rs" showLineNumbers startLineNumber=1 collapse={} ins={} 
//! Button module
...
```rust title="src/button/core.rs" showLineNumbers startLineNumber=20 collapse={} ins={} 
...
```rust title="src/button/core.rs" showLineNumbers startLineNumber=28 collapse={} ins={} 
impl<'a> Button<'a> {
    /// Constructor
    ///
...
```rust title="src/button/core.rs" showLineNumbers startLineNumber=55 collapse={} ins={} 
...
```rust title="src/button/consumer_loop.rs" showLineNumbers startLineNumber=1 collapse={} ins={} 
...
```rust title="src/button/mod.rs" showLineNumbers startLineNumber=1 collapse={} ins={} 
...
```txt frame="none" collapse={10-28} mark={33-36}
   ....
   Compiling big-button v0.1.0 (/home/you/projects/big-button)
    Finished `release` profile [optimized + debuginfo] target(s) in 0.77s
     Running `probe-rs run --chip=RP235x --log-format '[{t}] {[{L}]%dimmed%bold} {{f:dimmed}:{l:dimmed}%30}: {s}' target/thumbv8m.main-none-eabihf/release/big-button`
      Erasing ✔ 100% [####################]  12.00 KiB @  52.50 KiB/s (took 0s)
     Finished in 1.06s
