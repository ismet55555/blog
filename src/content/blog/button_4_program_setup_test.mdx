...
We are going to install all software needed to start interacting with our Raspberry Pi Pico W
microcontroller. We will setup everything for a simple [smoke test](https://en.wikipedia.org/wiki/Smoke_testing_(software)),
...
{/*
1. Basic microcontroller interaction software
2. Setup embassy project
3. Simple embassy code and run
*/}
...
Once Rust is installed we will have a few Rust [command line tools (CLI)](https://en.wikipedia.org/wiki/Command-line_interface)
...
Here is [way more information](https://rust-lang.github.io/rustup/index.html) about this tool.
...
To check if rust is already installed. Open up your computer terminal and enter the following
command. If you do not get a version number, you do not have Rust installed and will
have to install it.
...
- Using the installer script for Windows pre-installed [PowerShell](https://learn.microsoft.com/en-us/powershell/scripting/overview?view=powershell-7.4) terminal
...
rules seperately. udev rules are instructions that tell Linux what to do when specific devices
(like USB drives or debug probes) are plugged into your computer.
These rules control things like who can access the device, what name it gets,
...
Let's very our setup worked by connecting our debug probe device
to our computers USB port. For now, don't worry about the Raspberry
...
If you see that, great! You are done with this section. 
If you do not see that, try some of the follwoing steps.
