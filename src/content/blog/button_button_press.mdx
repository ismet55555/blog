...
## Introduction
<Tag color="green" txt="💻 HACKER" /> So far we have set up our program architecture
...
|-------------------|-------------|
| 🟢 <span class="text-green-600 font-bold">Short Press Release</span> | Button pressed and released *before* the long press threshold |
| 🟡 <span class="text-yellow-500 font-bold">Long Press Release</span> | Button pressed and released *after* the long press threshold but *before* the long hold threshold |
| 🔴 <span class="text-red-600 font-bold">Long Press Hold</span> | Button held continuously *beyond* the long hold threshold |
...
- **Press and release after 250ms** → 🟢 Short Press Release (released before 300ms threshold)
- **Press and release after 500ms** → 🟡 Long Press Release (released after 300ms but before 2000ms)
- **Press and hold for 2100ms** → 🔴 Long Press Hold (held beyond 2000ms threshold)
...
.
├── build.rs                  # <--- Changes: Add configs items
├── .cargo
├── Cargo.lock
├── Cargo.toml
├── configs.json              # <--- Changes: Add config items
├── .gitignore
├── memory.x
├── README.md
├── .rustfmt.toml
...
└── src
    ├── button
    │   ├── consumer_loop.rs
    │   ├── core.rs           # <--- Changes: Add press types
    │   ├── messaging.rs      # <--- Changes: Add press types
    │   └── mod.rs
    ├── clocks_config.rs
    ├── main.rs
    ├── state_machine.rs      # <--- Changes: Handle press types
    └── utility.rs
```
   ...
   Compiling big-button v0.1.0 (/home/you/projects/big-button)
    Finished `release` profile [optimized + debuginfo] target(s) in 0.77s
     Running `probe-rs run --chip=RP235x --log-format '[{t}] {[{L}]%dimmed%bold} {{f:dimmed}:{l:dimmed}%30}: {s}' target/thumbv8m.main-none-eabihf/release/big-button`
      Erasing ✔ 100% [####################]  24.00 KiB @  58.03 KiB/s (took 0s)
  Programming ✔ 100% [####################]  24.00 KiB @  27.60 KiB/s (took 1s)                                                                                                              
     Finished in 1.29s
...
1. **Short Press (57ms)**: The first press was quick—released before the 300ms threshold.
...
2. **Long Press (889ms)**: The second press was held longer—released after 300ms but
